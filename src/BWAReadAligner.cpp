...
bool BWAReadAligner::ProcessReadPair(ReadPair* read_pair) {
  if (debug) {
    cerr << "\n[BWAReadAligner]: processing "
         ...
         << " motif 2 " << read_pair->reads.at(1).repseq << endl;
  }
...
bool BWAReadAligner::ProcessRead(MSReadRecord* read,
  ...
  if (_bwt_references->find(repseq) == _bwt_references->end()) {
    if (align_debug) {
      cerr << "[ProcessRead]: repseq doesn't exist in ref" << endl;
    }
    return false;
  }
...
bool BWAReadAligner::StitchReads(ReadPair* read_pair,
  ...
  try {
    ...
    // If stitch across STR, set partial
...
int BWAReadAligner::GetMapq(const std::string& aligned_sw_string,
  ...
  // Add for clipped end gaps
  int begin_gap = 0;
  int end_gap = 0;
  size_t j = 0;
  char refchar = ref_sw_string.at(j);
  while (refchar == '-' && j < ref_sw_string.length() - 1) {
    begin_gap += 1;
    j++;
    refchar = ref_sw_string.at(j);
  }
  j = ref_sw_string.length() - 1;
  refchar = ref_sw_string.at(j);
  while (refchar == '-' && j > 0) {
    end_gap += 1;
    j--;
  ...
  score += (begin_gap+end_gap)*CLIP_PENALTY;
  if (debug_adjust) {
