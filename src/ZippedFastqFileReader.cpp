...
bool ZippedFastqFileReader::GetNextRead(MSReadRecord* read) {
  ...
  if (nuc.empty())
    errx(1, "Error: found empty nucleotide line in FASTQ file " \
         "'%s' line %zu",
         filename.c_str(), current_line);

  if (!valid_nucleotides_string(nuc))
    errx(1, "Error: found invalid nucleotide line in FASTQ file " \
         "'%s' line %zu",
         filename.c_str(), current_line);
  ...
  if (!getline(input_stream, quality)) {
    errx(1, "Error reading Quality scores line from FASTQ file " \
         "'%s' line %zu",
         filename.c_str(), current_line);
  }
