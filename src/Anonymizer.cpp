...
void Anonymizer::AnonymizeReads(const string& input_file,
  ...
  while(pReader->GetNextRecord(&ms_record)) {
	...
	read_id_to_alignment_.end()) {
      ...
      string name;
      AnonymizeSingleRead(&ms_record, &name);
      // and output ID/anonymized read
      string output_id = ms_record.ID + "\t" + name;
      idWriter.Write(output_id);
    }
    // output read
...
void Anonymizer::AnonymizeSingleRead(MSReadRecord* record_to_modify, string* name) {
  if (anonymizer_debug) {
    cerr << "Anonymizing " << record_to_modify->nucleotides << endl;
  }
  ...
  if (mask_n) {
    ...
    *name = aligned_read.STR_name;
    if (str_id_to_allele_nucs_.find(aligned_read.STR_name) == str_id_to_allele_nucs_.end()) {
      cerr << "STR " << *name <<" not in dictionary" << endl;
      return;
    }
...
bool Anonymizer::ParseAlignmentLine(const std::string& line, std::string* STR_name,
  ...
  *reverse_complement = false;
  ...
  //  int lstart = atoi(items[14].c_str());
  //int rstart = atoi(items[16].c_str());
  //*start_coord = lstart;
  //if (lstart > rstart) {
  //  *start_coord = rstart + 1;
  //}
  *start_coord = atoi(items[21].c_str());
  *STR_name = items[20];
  return true;
}
